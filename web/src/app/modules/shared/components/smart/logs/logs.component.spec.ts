...
describe('LogsComponent', () => {
  ...
  it('should stay at the bottom of the container when new logs arrive', () => {
    const nativeElement: ElementRef = component.scrollTarget;
    component.containerLogs = addLogsToList([], 15);
    fixture.detectChanges();
    expect(component.containerLogs.length).toBe(15);

    expect(nativeElement.nativeElement.classList).toContain(
      'container-content'
    );
    // expect(nativeElement.scrollTop).toEqual(0);

    // const prevScrollPosition = nativeElement.scrollTop;
    // component.containerLogs = addLogsToList(component.containerLogs, 300);
    // fixture.detectChanges();
    // expect(component.containerLogs.length).toBe(315);
    // console.log(nativeElement.scrollTop);
    // expect(nativeElement.scrollTop).toEqual(prevScrollPosition);
  });
